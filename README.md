...
遍历哔哩哔哩数字卡片收藏排行榜用户，保存其公开可见的卡片及编号，并查找你的“幸运号码”。
...
5. 选择快速测试、完整抓取或自定义范围；
6. 抓取完成后直接输入幸运号码查找所属用户。

用户卡片接口一次会返回该用户在当前收藏集内的全部卡片，因此即使选择某一张重点卡片，工具也会把其他卡片一起保存，不会增加请求次数。所选卡片用于限定幸运号码结果。
...
匹配结果可附加 `--out output/lucky.csv` 导出。所有卡片编号可直接导出：
